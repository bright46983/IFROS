...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def fs(self, xsk_1, usk):  # input velocity motion model with velocity noise
        ...
        **To be completed by the student**.
        ...
        :parameter xsk_1: previous robot state :math:`x_{s_{k-1}}=\\begin{bddmatrix}\\eta_{s_{k-1}}^T & \\nu_{s_{k-1}}^T\\end{bmatrix}^T`
        ...
        #
        pos_1 = Pose3D(np.array([[xsk_1[0,0]], [xsk_1[1,0]],[xsk_1[2,0]]]))
    ...
    def ReadCompass(self):
        ...
        # TODO: to be completed by the student
        yaw = np.array([self.xsk[5,0]])
        R_yaw = np.array([self.v_yaw_std**2])
        return yaw , R_yaw

    def PlotRobot(self):
        """ Updates the plot of the robot at the current pose """

        self.vehicleIcon.update([self.xsk[0,0], self.xsk[1,0], self.xsk[2,0]])
        plt.pause(0.0000001)
        return

if __name__== '__main__':
    initial_state = np.zeros((6,1))
    desired_vel = np.array([[0.05],[0.005]])
    M2D = [np.array([[-40, 5]]).T,
           np.array([[-5, 40]]).T,
           np.array([[-5, 25]]).T,
           np.array([[-3, 50]]).T,
           np.array([[-20, 3]]).T,
           np.array([[40,-40]]).T]
    robot = DifferentialDriveSimulatedRobot(initial_state,M2D)
    current_state = robot.fs(initial_state,desired_vel)
    try:
        while True:
