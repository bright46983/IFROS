...
def kinematics(d, theta, a, alpha):
    ...
    for i in range(len(d)):
        dh = DH(d[i],theta[i],a[i],alpha[i]) # Compute the DH transformation from joint i-1 to i.
        T.append(T[-1] @ dh) # Compute transformation from base to joint i by multiplying the lastest trasformation
    return T
